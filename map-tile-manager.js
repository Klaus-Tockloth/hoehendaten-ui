...
function createTileManager(config) {
  ...
  async function revisitTileIndices(copiedTilesMap = []) {
    ...
    // Get the total number of entries to use in the progress display
    ...
    for (const [key, tilesArray] of copiedTilesMap.entries()) {     
      ...
      await handleGoButtonForMode(effectiveLatLng, progressText);
    ...
    document
      ...
      .addEventListener("click", async () => {       
        ...
        try {
          ...
          const formattedBounds = `${south}, ${west}\n${north}, ${east}`;

          alert(`Kacheln geladen für den Bereich:\n${formattedBounds}`);
        } catch (error) {
